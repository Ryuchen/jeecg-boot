...
import { cityMap, provinceMap, special } from '@assets/maps/index'

let theme = require('@assets/maps/themes/theme.json');
      ...
      return {
        ...
        series: [
          ...
          },
          {
            ...
            type: 'effectScatter',
            coordinateSystem: 'geo',
            data: this.convertData(this.data.sort((a, b) => b.value - a.value).slice(0, 6)),
            symbolSize: val => val[2] / 10,
            showEffectOn: 'render',
            rippleEffect: {
              brushType: 'stroke'
  ...
  methods: {
    ...
    clickHandler (params) {
      ...
      if (name in this.provinceMap) {
        ...
        this.mapName = name
        this.updateChart()
      } else if (name in this.cityMap) {
        // 如果是【直辖市/特别行政区】只有二级下钻
        if (this.special.indexOf(name) >= 0) {
          this.mapName = name
          this.updateChart()
        } else {
          // 显示县级地图
          let city = require(`@assets/maps/city/${this.cityMap[name]}.json`)
    ...
    init () {
      // 初始化 echarts
      this.myChart = echarts.init(this.$refs.map, 'ovilia-green')
      this.myChart.setOption(this.opt)
